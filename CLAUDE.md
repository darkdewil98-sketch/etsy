...
- `research/<date>/` holds dated research runs (Trends, Printful costs, USPTO screens).
  ...
  score_opportunities.py -> build_report.py. Round 3 + 4 data applied; finalist IP screen in data/tm_round3_*.{csv,txt} ("PET PILLOW" is registered -> pillow lead keyword is "custom dog pillow"). Next data request: chrome_prompt_round5_prices.md. Earlier: (data/round3_*.txt, data/round4_*.txt, data/pillow_16in_prices.csv; run scripts/pillow_16in.py before scoring). Ornaments are scored on the verified SwiftPOD 2-side cost ($6.57 + $4.79).
